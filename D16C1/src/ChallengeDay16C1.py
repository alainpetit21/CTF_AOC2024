...
from D16C1.src.Tile import Tile
from engine.Challenge import Challenge


class ChallengeDay16C1(Challenge):
    ...
    def interpret_data(self, data_inj: [str] = None):
        ...
        for y in range(len(self.tilemap)):
            for x in range(len(self.tilemap[0])):
                if self.data[y][x] == '#':
                    self.tilemap[y][x].is_wall = True
                    continue

                if self.data[y][x] == 'S':
                    self.tilemap[y][x].is_start = True
                    self.start = (x, y)

                if self.data[y][x] == 'E':
                    self.tilemap[y][x].is_goal = True
                    self.goal = (x, y)

                if (x < width_tiles - 1) and (self.data[y][x + 1] != '#'):
                    self.tilemap[y][x].set_east(self.tilemap[y][x + 1])

                if (y < height_tiles - 1) and (self.data[y + 1][x] != '#'):
                    self.tilemap[y][x].set_south(self.tilemap[y + 1][x])

                if (x > 0) and (self.data[y][x - 1] != '#'):
                    self.tilemap[y][x].set_west(self.tilemap[y][x - 1])

                if (y > 0) and (self.data[y - 1][x] != '#'):
                    self.tilemap[y][x].set_north(self.tilemap[y - 1][x])
        ...
        while queue:
            ...
            for neighbor in current_tile.neighbor:
                if neighbor is not None:
                    ...
                    # Calculate angle turn cost
                    turn_cost = 0
                    if current_direction is not None and direction != current_direction:
                        turn_cost = 100  # Cost of a 90-degree turn

                    # Assuming each move has a cost of 1
                    distance = current_distance + 1 + turn_cost
    ...
    def run(self):
        ...
        # print(f"NbStep: {nb_step - 1}; NbRotations: {nb_rotation}")

        self.result = nb_rotation*1000 + nb_step-1
        return self.result
