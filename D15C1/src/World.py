...
class World:
    ...
    def load_data(self, data):
        for y in range(World.HEIGHT):
            for x in range(World.WIDTH):
                match data[y][x]:
                    case '#':
                        self.internal_world[y][x] = Wall(x, y, self)
                        self.walls.append(self.internal_world[y][x])
                    case 'O':
                        self.internal_world[y][x] = Box(x, y, self)
                        self.boxes.append(self.internal_world[y][x])
                    case '@':
                        self.internal_world[y][x] = Robot(x, y, self)
                        self.the_robot = self.internal_world[y][x]
    ...
    def __str__(self):
        ...
        for y in range(World.HEIGHT):
            for x in range(World.WIDTH):
                object_at = self.internal_world[y][x]
